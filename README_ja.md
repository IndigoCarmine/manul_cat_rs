...
* **PLUMED 入力の確認**（`plumed.dat`）— PLUMED スクリプトを開くと左パネルに
  PLUMED タブが増え、ファイル全体がシンタックスハイライト付きの行リストとして
  並ぶ。上下キーまたは Prev / Next ボタンで行を順に送ると、いま立っている行が
  3D ビュー上に現れる。対象原子はハロー表示、`CENTER` / `COM` の仮想原子には
  マーカー球、CV が測っているベクトルは矢印で描かれる。bias や関数は `ARG=` を
  辿って参照先の CV の形状を引き継いで表示する。構造に存在しない原子番号は
  黙って捨てず、その行のエラーとして赤く示す — 設定ミスの検出がこの機能の目的。
  ファイルへの書き込みは一切行わない。
* **XYZ スライス** — 右パネル下半分に軸ごとの切断面スライダ。軸を有効にすると、
  その座標が面のどちら側にあるかで原子の表示が決まる。三軸を組み合わせれば
  面でも角でも削り取れるので、溶媒に埋もれた中心部をそのまま覗ける。切断は
  画像・動画の書き出しと他レイヤーのオーバーレイ表示にも反映され、トラジェクトリ
  再生中も面は空間に固定されたまま原子だけがそこを通過する。表示上の状態なので、
  構造ファイルの書き出しは常に全原子。
