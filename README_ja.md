...
* **XTC トラジェクトリ再生** — 再生 / コマ送り / シーク、FPS 調整、フレーム間の
  補間（スムージング）。
* **動画書き出し** — 読み込んだトラジェクトリを MP4 に書き出す。エンコーダは同梱せず、
  PNG 連番を自前で描画して手元の `ffmpeg` に渡す方式。`ffmpeg` が無くても連番は残る。
  Windows では実行コマンドを提示した上で `winget` 経由の導入を代行でき、それ以外の
  OS では `apt` / `dnf` / `flatpak` / `brew` のコマンドを案内する。
